...
-- Everyone can view profiles (non-sensitive)
...
-- Public can view anonymous listing metadata
...
-- Referrers can see their own referrals. Listing owners can see referrals for their listings.
create policy if not exists "Referrers and listing owners can view referrals"
...
create policy if not exists "Users can create their own referrals"
  on public.referrals for insert
  with check (referrer_id = auth.uid());

-- Optional: allow referrers to delete their own entries
create policy if not exists "Referrers can delete their own referrals"
