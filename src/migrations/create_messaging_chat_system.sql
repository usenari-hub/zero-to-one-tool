...
-- Run this SQL in your Supabase SQL editor to create the new messaging system
...
CREATE INDEX idx_chat_listing ON listing_chat_messages(listing_id);
CREATE INDEX idx_chat_sender ON listing_chat_messages(sender_id);
CREATE INDEX idx_chat_status ON listing_chat_messages(status);
CREATE INDEX idx_chat_created ON listing_chat_messages(created_at);
...
CREATE INDEX idx_direct_listing ON direct_messages(listing_id);
CREATE INDEX idx_direct_sender ON direct_messages(sender_id);
CREATE INDEX idx_direct_recipient ON direct_messages(recipient_id);
CREATE INDEX idx_direct_created ON direct_messages(created_at);
...
CREATE INDEX idx_reports_severity ON user_reports(severity);
CREATE INDEX idx_reports_created ON user_reports(created_at);

-- Prevent duplicate reports
ALTER TABLE user_reports ADD CONSTRAINT unique_report_per_message UNIQUE(reporter_id, message_id);
...
CREATE INDEX idx_violations_reviewed ON content_violations(reviewed);
CREATE INDEX idx_violations_created ON content_violations(created_at);
...
-- Create indexes for charity fund
CREATE INDEX idx_charity_fund_listing ON charity_fund(listing_id);
CREATE INDEX idx_charity_fund_disbursed ON charity_fund(disbursed);
CREATE INDEX idx_charity_fund_allocated_at ON charity_fund(allocated_at);

-- Ensure one entry per purchase
ALTER TABLE charity_fund ADD CONSTRAINT unique_purchase_charity UNIQUE(purchase_id);
